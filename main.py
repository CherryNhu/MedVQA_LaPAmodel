...
# FIXING
import torch
# Clear GPU memory cache before starting the training process
...
def main(_config):
    ...
    wb_logger = pl.loggers.WandbLogger(project="DI-VQA", name=run_name)
    ...
    num_gpus = (_config["num_gpus"] if isinstance(_config["num_gpus"], int) else len(_config["num_gpus"])) # 1
    grad_steps = max(_config["batch_size"] // (_config["per_gpu_batchsize"] * num_gpus * _config["num_nodes"]), 1) # 32
    max_steps = _config["max_steps"] if _config["max_steps"] is not None else None # 100000
    max_epochs = _config["max_epoch"] if max_steps is None else 1000 # 1000
